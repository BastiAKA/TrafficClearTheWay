namespace ClearTheWay
{
    ...
    internal static class Tuning
    {
        ...
        internal const int kMaxCorridorLanes = 24;       // most lanes ONE responder ever parts in a tick. kChannelMaxDistance bounds the channel's share, but nothing bounded the total: master lane groups at a junction each expand to their full set, so the count ran to 27 and then 62 (veh 2350532) - each one a full LaneObject scan per responder per tick, and cars shoved that are nowhere near the responder's path. A normal corridor is 2-6 lanes, so this only bites on the runaway cases.
        ...
        internal const float kChannelReleaseSpeed = 16f; // ...but a responder ALREADY running a wide-road shape keeps it until this speed. A bare threshold has no width: a responder hovering around kChannelMaxSpeed rebuilt its corridor in a different SHAPE every tick - the traffic around it got opposite push directions in consecutive ticks (the push claim then simply blocked the second one for 2 s) and its own hug target jumped between the middle seam and the corridor edge. Enter slow, leave clearly faster.
        internal const uint kShapeCommitFrames = 180u;   // ~3 s: how long a chosen corridor shape is held before it may be re-planned. REFRESHED every tick cars are actually being pushed for it (EscalationCorridor), so this is not a countdown under a formed Rettungsgasse - a working corridor keeps its shape indefinitely and the dwell only decides how long after the last push the decision stays frozen. Sebastian: once a gasse has been built the behaviour has to stay consistent; re-planning underneath it tears up the very gap the queue just opened.
        internal const uint kConvoyHoldFrames = 120u;    // ~2 s: convoy discipline is held this long after the last sighting of a colleague ahead. Same reason as kHugHoldFrames - the raw 20 m test flips as the colleague pulls away and closes up, and each flip swung the evade stage between Soft and Hard and the hug between the edge line and the full swing.
        internal const float kStuckProgressMeters = 25f; // a blocked spell ends when the responder has covered this much GROUND, not when it briefly rolls. The old test was instantaneous speed (< kMaxSqueezeSpeed): in stop-and-go a single creep over 5 m/s reset m_BlockerSinceFrame and with it EVERY escalation clock (hard evade 150f, desperate 600f, deep evade 1800f, the 3-minute light rule), so they almost never matured where they were needed most. 25 m is roughly five car lengths - far enough that the responder has genuinely got past whatever was in front, and at a 1 m/s jam creep it takes 25 s, so the escalations still mature. Exactly the lesson already recorded at kAssistProgressMeters for the recovery side, applied to the responder itself.
        ...
        internal const float kLateralLeadFactor = 2f;    // a steering target that realises a lateral offset is placed at least this many times the OFFSET ahead of the vehicle, so the bearing to it stays under ~27 degrees. A vehicle does not step sideways - it drives diagonally - and at a steep bearing it resolves the target by rotating on the spot instead ("dreht sich im Stand", veh 2350532 at lanePos 4.89 with the old fixed 2 m lead: ~66 degrees). The 35-degree clamp in TryGetLateralTarget is the safety net; this is what keeps it from firing in the first place.
        ...
        internal const uint kNearWreckSweepInterval = 8u;   // how long the near-wreck MEMBERSHIP list is reused (~0.14 s). The sweep that builds it is a 600 m quadtree traversal per accident cluster PLUS a main-thread job sync, and it ran every single tick - the one major pass in the mod with no throttle at all, while Pedestrians (10), the beacon scan (4), LateralRoom (64) and the orphan finder (128) all had one. PathEndGuardSystem already argues the case for reuse in its own comment: cars move well under a metre per tick, so the SET is identical across a handful of them. Only membership is cached - the protection work itself (despawn shield, repath shield, approach brake) still runs on every car every tick, so behaviour is unchanged.
        internal const uint kResponderShieldInterval = 4u;  // how often the whole-fleet PathfindFailed shield sweeps EVERY police/ambulance/fire vehicle in the city (~69 ms). The pass is already gated on an accident existing, but in a large city that is the permanent state, so it effectively ran every tick to do nothing for hundreds of vehicles. ASSUMPTION worth re-checking: vanilla's vehicle AI systems update on a multi-frame phase, so a 4-frame cadence still clears Failed|Stuck long before the AI's delete branch can see them. If a responder is ever observed despawning at a blocked accident again, this is the first constant to put back to 1.
        ...
        internal const int kPlugChainHops = 8;             // how far up the blocker chain the crosswalk guard looks for the head of the column. A responder is rarely more than a handful of cars behind the one actually yielding, and the walk is bounded rather than cycle-tracked on purpose (see PedestrianPlug.FindPlugCreature) - a cycle just burns the budget and answers "no pedestrian", which is the safe answer.
        internal const uint kPedPlugFrames = 1800u;        // ~30 s: the SAME pedestrian holding the head of the column this long is not a crossing pulse any more, it is a stuck pedestrian. Pulses at a busy zebra last seconds, so this is deliberately far beyond one - the cheap outcome (just wait, it clears itself) is the right one in every ordinary case.
        internal const uint kPedRepathCooldown = 900u;     // ... and such a pedestrian is asked to re-path at most once per this many frames, the same throttle the car-side defreeze uses, so a crossing full of people cannot flood the pathfinder.
        internal const uint kPedPlugGiveUpFrames = 5400u;  // ~90 s: the forced re-path did not move it either. Stop protecting the crossing and let the ordinary car sacrifice run - by then the responder has stood for kLightOverrideFrames PLUS this, and whatever is going on is not the pedestrian pulse this guard was built for.
        ...
        internal const float kAssistRollingSpeed = 4f;         // ~14 km/h: above this the recovery vehicle is genuinely driving, not creeping forward in a stop-and-go queue. Only used as the RELEASE test for the escalation latch, never to start one.
        internal const uint kAssistRightsHoldFrames = 2610u;   // ~45 s: once a recovery vehicle has earned the escalation it keeps it until it has driven freely for this long. Without the latch every stage hung directly off timers that reset on the first metre of progress, so in stop-and-go the vehicle gained its rights, rolled one car length, lost them, stopped again - and the corridor it had just built was torn up each time. Same reasoning as kSqueezeLatchFrames / kShapeCommitFrames / kFreeLaneLatchFrames elsewhere: a decision that flips every few seconds is worse than either of its outcomes. Sebastian: once it has special rights it must drive 30-60 s before losing them again.
        internal const float kAssistMotionlessMeters = 3f;     // a recovery vehicle that has not covered this much ground is not moving. The same 3 m the wedge check uses, and deliberately a DISTANCE test rather than a speed one - in stop-and-go traffic an instantaneous speed reads non-zero constantly while the vehicle goes nowhere.
        internal const uint kAssistMotionlessFrames = 1800u;   // ~30 s without covering kAssistMotionlessMeters => wedged, at ANY distance from the wreck. This is the gate that grants the FULL escalation at any distance: kAssistEscalateRange is right that straight-line distance is no progress measure out on the route, but that left a truck standing in an ordinary jam 800 m out with no help at all - no corridor, no lane change, not even a log line (truck 1824444: 13 wedge strikes, four minutes motionless, zero [assist] lines, then sent home). Physical movement is the metric that holds everywhere.
        ...
        internal const uint kUnblockStableFrames = 300u;   // ~5 s: the blocker in front of a wedged recovery vehicle must be the SAME entity this long before it may be flagged or removed. Two cars standing abreast make the game's Blocker alternate between them every few seconds (it clears IgnoreBlocker on every change, which is why neither gets pushed past), and without this gate the unblock fires at whichever one happened to be current - a stranger's car deleted for a deadlock it was only half of.
        internal const uint kAssistFullRightsFrames = 3480u;  // ~1 min without progress and the recovery vehicle stops asking nicely: its light petitions go from kAssistSignalPriority (106, effective at the junction's next phase switch) to kSignalPriority (108) WITH preemption - the lane is hard-set to Go, exactly as it is for an ambulance. Staged rather than granted up front because cutting a running green phase short is a real cost to everyone else at that junction, and most recovery runs never need it; a truck wedged for a full minute beside a wreck has already demonstrated that politeness is not working. Sebastian set this to 1 min after watching a 570k city where the polite tier never once broke a truck loose - at that scale the jam a wreck creates does not clear itself inside three. Bounded by the same near-wreck gate (kAssistEscalateRange) as the rest of the escalation, so a truck out on the route can never reach this tier.
        internal const uint kAssistGiveUpFrames = 34800u;  // ... and once even escalation buys NO progress for this long (~90 s), the recovery is wedged in an unwinnable deadlock (a vanilla gridlock the toolkit cannot break - one truck escalated 364x over 152 s stuck at a fixed 30 m). Stop escalating: it only churns surrounding traffic for nothing, and the wreck despawns via give-up @kWreckGiveUpAge anyway. Logged in full detail so the deadlock itself could be tackled later.
        ...
        internal const float kOncomingDesperateCommitSight = 40f; // ...but a DESPERATE responder (kDesperateFrames, ~10 s behind the same blocker with no way through) commits on this much. This is the rule kDesperateFrames has promised all along - "stuck this long => cross over even against oncoming traffic" - and it was never actually implemented: the desperate flag was passed into TryOncomingDisplacement and the body simply ignored it, so a responder that had been standing for a minute needed exactly the same 75 m gap as one that had just started evading, and in city traffic it therefore never crossed at all (field case veh 2350571: onc=0 in every single log line). Deliberately kept well above kOncomingHardMergeSight (25 m), so it can never commit into a car it would have to merge away from on the next tick.
        ...
        internal const float kFarVanRange = 500f;     // ... and only past THIS does the wreck go back to the tow depot. The 100 m gap is hysteresis, and it is not cosmetic: with a single threshold a wreck sitting at ~400 m alternates between the two dispatch paths every cooldown, because the two paths compete for the same maintenance request (the depot claims it, the van's order evaporates, the wreck looks unassigned, repeat). One metre of distance must never be able to switch the path.
        ...
        internal const uint kDepotClaimFrames = 10800u;    // ~3 min: after handing a wreck's request to a tow depot, the dispatcher does not hand it over AGAIN for this long. A depot consumes the request immediately and spawns a truck, so the "already queued there?" test cannot see the in-flight order, and the truck itself stays invisible to VanQuery while it is still parked in the depot. Without this the wreck is re-dispatched every kActionCooldown and the depot answers each time with another truck. Sized to cover spawn + unpark + a drive across town; the wreck's own give-up watchdogs remain the backstop if the truck never arrives.
        internal const uint kVanClaimFrames = 10800u;      // ~3 min: how long the dispatcher defends its OWN van assignment against the depot branch (TowAssignment.OwnVanStillComing). Generous on purpose - a van called from up to kNearVanRange away through city traffic legitimately takes minutes, and the whole point is to stop re-deciding before it arrives. It is only a backstop anyway: three live checks (van gone / re-targeted / no longer suitable) drop the claim long before this in every ordinary case.
